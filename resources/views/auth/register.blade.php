...
<x-guest-layout>
    <x-jet-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                <x-jet-label for="current_team_id" value="{{ __('Choose Team') }}" />
                <select class="block w-full mt-1 border-gray-300 rounded-md focus:border-gray-400 focus:outline-none"
                    name="current_team_id" id="current_team_id" required>
                    <option value="">Select One</option>
                    @forelse ($teams as $team)
                        <option value="{{ $team->id }}"
                            {{ old('current_team_id') == $team->id ? 'selected' : '' }}>
                            {{ $team->name }}
                        </option>
                    @empty
                ...
                {{-- <x-select id="current_team_id" class="block w-full mt-1" type="text" name="current_team_id"
                    :value="old('current_team_id')" required /> --}}
            </div>
